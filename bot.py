...
def approve_or_disapprove_user(message):
    ...
    if not is_admin:
        bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                   "*You don't have permission to use this command Contact Owner @pro_godzila_op.*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact Owner @pro_godzila_op.*", parse_mode='Markdown')
            return
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please Contact Owner @pro_godzila_op* "
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"
        response += "*Please Contact Owner @pro_godzila_op* 🙏"
    else:
